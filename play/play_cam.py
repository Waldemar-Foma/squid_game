...
# Окно с запросом о наличии камеры
...
# Запуск окна с запросом о камере
show_camera_prompt()
...
# Создаем окно Tkinter для отображения камеры
...
# Запуск окна Tkinter в отдельном потоке
camera_thread = threading.Thread(target=start_camera_window)
